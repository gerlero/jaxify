...
class _Transformer(ast.NodeTransformer):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> ast.FunctionDef:
        if self.__top_level_function:
            ...
            node.body.insert(
                0,
                ast.Assign(
                    targets=[ast.Name(id="__jaxify_return__", ctx=ast.Store())],
                    value=ast.Call(
                        func=ast.Attribute(
                            value=ast.Name(id="__jaxify_hooks__", ctx=ast.Load()),
                            attr="Return",
                            ctx=ast.Load(),
                        ),
                        args=[],
            ...
            node.body.append(
                ast.Return(
                    value=ast.Attribute(
                        ast.Name(id="__jaxify_return__", ctx=ast.Load()),
                        attr="value",
                        ctx=ast.Load(),
                    )
                )
    ...
    def visit_If(self, node: ast.If) -> list[ast.stmt]:
        ...
        if (
            "__jaxify_return__" in written_vars_body
            and "__jaxify_return__" not in written_vars_orelse
        ):
            read_vars.add("__jaxify_return__")
            type_stable_return_assignments_body = []
            type_stable_return_assignments_orelse = [
                ast.Assign(
                    targets=[ast.Name("__jaxify_return__", ast.Store())],
                    value=ast.Call(
                        ast.Attribute(
                            ast.Name(id="__jaxify_return__", ctx=ast.Load()),
                            attr="set_type",
                        ...
                        args=[
                            ast.Attribute(
                                value=ast.Subscript(
                                    ast.Call(
                                        ast.Name(
                                            f"__jaxify_if_true_{self._if_count}__",
                                            ast.Load(),
                                        ),
                                        [
                                    ...
                                    ast.Constant(
                                        list(written_vars).index("__jaxify_return__")
                                    ),
                                    ast.Load(),
                                ),
                                attr="value",
            ...
            "__jaxify_return__" in written_vars_orelse
            and "__jaxify_return__" not in written_vars_body
        ):
            read_vars.add("__jaxify_return__")
            type_stable_return_assignments_body = [
                ast.Assign(
                    ...
                    value=ast.Call(
                        ...
                        args=[
                            ast.Subscript(
                                ast.Call(
                                    ast.Name(
                                        f"__jaxify_if_false_{self._if_count}__",
                                        ast.Load(),
                                    ),
                                ...
                                ast.Constant(
                                    list(written_vars).index("__jaxify_return_value__")
                                ),
                                ast.Load(),
                            )
                        ],
        ...
        if_true = ast.FunctionDef(
            name=f"__jaxify_if_true_{self._if_count}__",
            args=args,
            body=[
                *node.body,
                *type_stable_return_assignments_body,
        ...
        if_false = ast.FunctionDef(
            name=f"__jaxify_if_false_{self._if_count}__",
            args=args,
            body=[
                *node.orelse,
                *type_stable_return_assignments_orelse,
        ...
        new_if = ast.Assign(
            ...
            value=ast.Call(
                ...
                args=[
                    node.test,
                    ast.Name(f"__jaxify_if_true_{self._if_count}__", ast.Load()),
                    ast.Name(f"__jaxify_if_false_{self._if_count}__", ast.Load()),
                    *[ast.Name(var, ast.Load()) for var in read_vars],
                ],
                keywords=[],
            ),
...
def jaxify(func: Callable[_Inputs, _Output], /) -> Callable[_Inputs, _Output]:
    ...
    exec(  # noqa: S102
        ...
        {
            **func.__globals__,
            "__jaxify_hooks__": importlib.import_module("jaxify._hooks"),
        },
        local_vars,
    )
