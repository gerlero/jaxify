...
@overload
def jitx(
    func: Callable[_Inputs, _Output], /, *, vmap: bool = ...
) -> Callable[_Inputs, _Output]: ...


@overload
def jitx(
    *, vmap: bool = ...
) -> Callable[[Callable[_Inputs, _Output]], Callable[_Inputs, _Output]]: ...


def jitx(  # noqa: C901
    func: Callable[_Inputs, _Output] | None = None, /, *, vmap: bool = False
) -> (
    Callable[_Inputs, _Output]
    | Callable[[Callable[_Inputs, _Output]], Callable[_Inputs, _Output]]
):
    def jitx_decorator(  # noqa: C901
        func: Callable[_Inputs, _Output], /
    ) -> Callable[_Inputs, _Output]:
        ...
        def jitx_wrapper(*args: _Inputs.args, **kwargs: _Inputs.kwargs) -> _Output:  # noqa: C901
            ...
            for combination in cond_combinations:
                ...
                def _jaxify_cond(cond: object, cond_id: int) -> bool:
                    if isinstance(
                        cond, jax.interpreters.partial_eval.DynamicJaxprTracer
                    ):
        ...
        return jax.jit(jax.vmap(jitx_wrapper)) if vmap else jitx_wrapper

    return jitx_decorator(func) if func is not None else jitx_decorator
